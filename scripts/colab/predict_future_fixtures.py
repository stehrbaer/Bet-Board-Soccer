...
def build_future_feature_frame(fixtures: pd.DataFrame, feature_names: list[str], snapshots: dict[str, dict[str, Any]]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if "game_week" not in fixtures.columns:
        fixtures["game_week"] = fixtures["matchweek"]
    if "relative_week" not in fixtures.columns:
        relative_week_numbers = {week: idx for idx, week in enumerate(fixtures["week_start"].drop_duplicates(), start=1)}
        fixtures["relative_week"] = fixtures["week_start"].map(relative_week_numbers)
        ...
        row.update(
            {
                ...
                "game_week": fixture.get("game_week"),
                "matchweek": fixture.get("matchweek"),
                "relative_week": fixture.get("relative_week"),
                ...
                "matchup_key": f"{fixture.get('game_week')}_{slug(fixture.get('home_team_name'))}_{slug(fixture.get('away_team_name'))}",
            }
        )
        rows.append(row)
